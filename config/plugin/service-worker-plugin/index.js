...
class ServiceWorkerPlugin {
  ...
  apply(compiler, options = this.defaultOptions) {
    ...
    compiler.hooks.compilation.tap(ServiceWorkerPlugin.name, (compilation, callback) => {
      // 将逻辑注册在同一个 ServiceWorkerPlugin 方法上
      HtmlWebpackPlugin.getHooks(compilation).afterTemplateExecution.tap(ServiceWorkerPlugin.name, (res) => {
        // 插入内嵌代码
      ...
      compilation.hooks.processAssets.tapAsync(
        ...
        (assets, callback) => {
          ...
          if (assetSkeys.length) {
            ...
            let minifyCode = this.minifyJsCode(filePath, { cacheFileList: JSON.stringify(assetSkeys) });
            // 将JavaScript文件添加到Webpack的输出中
            assets[this.outputName] = new webpack.sources.RawSource(minifyCode)
            // 写入打包时间
            assets['version'] = new webpack.sources.RawSource(String(this.buildTime))
          }
          callback()
        }
      )
  ...
  minifyJsCode(path, params = {}) {
    try {
      let fileContent = this.replaceText(fs.readFileSync(path, { encoding: 'utf-8' }), params)
      // 壓縮代碼
      let minify = uglify.minify(fileContent, {
        mangle: true,
        compress: {
          drop_console: false,
        },
