...
void main () {
  ...
  vec4 rgb = texture2D(u_image_rgb, v_texCood);
  vec4 alpha = texture2D(u_image_alpha, v_texCood);

  // 在纹理上寻找对应颜色值
   gl_FragColor =  rgb * 0.5 + alpha * 0.5;
}`;
...
class VapGl {
  ...
  initCanvas() {
    ...
    // 创建缓冲区
    var texcoordBuffer = this.webgl.createBuffer();
    // 绑定缓冲区
