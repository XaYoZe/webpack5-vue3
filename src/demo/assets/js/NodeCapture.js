export default class NodeCapture {
  ...
  async createStyle(computedStyle, el) {
    ...
    while ((styleName = computedStyle.item(count))) {
      ...
      if (
        el &&
        styleText === this.defaultStyle[el.tagName]?.getPropertyValue(styleName)
      )
        continue;
  ...
  async hasPseudoElement(node, tag) {
    ...
    if (beforeStyle.content !== "none") {
      beforeStyleText = `${selector}::before { ${await this.createStyle(
        beforeStyle
      )} }`;
    }
    if (afterStyle.content !== "none") {
      afterStyleText = `${selector}::after { ${await this.createStyle(
        afterStyle
      )} }`;
    }
  ...
  async compile(node, conf, layer = "1") {
    ...
    if (node.nodeType == 1) {
      ...
      let style = node.computedStyleMap();
      let styleTag = document.createElement("style");
      let styleText = await this.hasPseudoElement(node, tag);
      styleTag.innerHTML = styleText + `${await this.createStyle1(style, tag)}`;
  ...
  async createStyle1(styleMap, el) {
    return new Promise(resolve => {
      ...
      styleMap.forEach(async (cssVal, cssKey) => {
        if (
          this.defaultStyleMap[el.tagName]?.get(cssKey)?.toString() !==
          cssVal.toString()
        ) {
          let styleText = cssVal.toString();
          // console.log(cssKey, this.defaultStyleMap[el.tagName].get(cssKey).toString(), '->', cssVal.toString());
          if (/url\(.*\)/.test(styleText) && !/url\(data:.*\)/.test(styleText)) {
            imgTotalCount++;
            let urlReg = /url\(["'](.*?)['"]\)/g;
            let newStyle = styleText;
            ...
            while ((urlExec = urlReg.exec(styleText))) {
              try {
                let url = urlExec[1];
                let res = await this.getPic(url);
                newStyle = newStyle.replace(url, res);
              } catch (err) {}
            }
            imgCount++;
            styleText = newStyle;
  ...
  createCanvas(image) {
    ...
    canvas.height =
      (this.height || image.offsetWidth) * window.devicePixelRatio;
  ...
  async shot() {
    ...
    let canvasData = await this.createCanvas(img);
